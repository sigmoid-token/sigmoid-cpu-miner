...
void Solver::setParameters(int s_cost, int t_cost, int delta) {
  // mutex
  this->s_cost = s_cost;
  this->t_cost = t_cost;
  this->delta = delta;
...
void Solver::setWhetherBalloon(bool is_balloon) {
  this->is_balloon = is_balloon;
  updateBuffer();
}
...
void Solver::updateBuffer()
{
  ...
  //  to acquire a lock on each hash() loop
  {
    ...
    balloon_hasher.s_cost = this->s_cost;
    balloon_hasher.t_cost = this->t_cost;
    balloon_hasher.delta = this->delta;
  }
  m_buffer_ready = true;
}
...
void Solver::hash(bytes_t const& solution, bytes_t& digest)
{
  ...
  if (is_balloon) {
    balloon_hasher.balloon_hash(&digest[0], digest.size(), &solution[0], solution.size(), &m_buffer[0], m_buffer.size());
  }
  else {
