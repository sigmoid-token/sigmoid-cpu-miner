...
void CpuMiner::setParameters(int s_cost, int t_cost, int delta)
{
  for (auto&& i : m_solvers)
    i.setParameters(s_cost, t_cost, delta);
}

void CpuMiner::setWhetherBalloon(bool is_balloon)
{
  for (auto&& i : m_solvers)
    i.setWhetherBalloon(is_balloon);
}
